<script>
	...
	onMount(async () => {
		splittedTitle = await data.primary.secondary_hero_title[0].text.split(' ');
	});
</script>

<div class="relative w-full h-960">
	<div class="text-wrapper w-full h-full pb-72">
		<div class="relative w-full h-full">
			...
			<div class="absolute left-32 bottom-4 text-primary-main text-h2 font-bold">
				<Divider name="section-cosmovision-2-divider" w={'420'} h={'32'} pixCol={'#BDFF00'} />
				{#each splittedTitle as word}
					<h2>{word}</h2>
				{/each}
			</div>
		</div>
